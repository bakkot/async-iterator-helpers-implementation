...
source — `it.return()` is not called — but it is always a **wall**: it makes every
*later* in-flight pull unobservable. Positions after the done are discarded — even
ones that already settled with a value or an error — and a value arriving later for
such a position is ignored without invoking `pred`. This holds whether the source
finished on its own or is draining an `it.return()` we issued (§6).
...
**A done overrides every later position.** A done discards every later position,
including one that already errored (e.g. a pull that threw synchronously before the
done arrived) or already holds a kept value: those positions are beyond the
sequence's end, so their outcomes are suppressed — not resurrected, even if an
earlier value is later dropped. This holds for *every* done, regardless of which
terminal event (if any) preceded it: a source error does **not** close the source, so
a subsequent source done is a wall; and a done the source produces *after* we closed
it via `it.return()` — draining a predicate error (§6) or a `return()` (§7) — is a
wall too. In particular, when a source reacts to `it.return()` by resolving an earlier
outstanding pull with done, that done discards the very predicate error that triggered
the close, silently **swallowing** it: the consumer sees done rather than the error.
Treating a done uniformly as a wall is what lets the result avoid tracking whether it
closed the source — the open/closed distinction makes no difference to delivery.
...
8. A `done` is always a terminal wall: it discards its position and every later one,
   including a later position that already settled with a value or an error, no matter
   which terminal event (if any) preceded it. A done the source produces while draining
   an `it.return()` can therefore swallow an already-determined later value or error
   (§4, §6).
